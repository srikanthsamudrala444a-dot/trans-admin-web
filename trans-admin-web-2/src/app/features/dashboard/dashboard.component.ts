...
import { PassengerChartComponent } from "../passengers-chart/passengers-chart.component";
...
export class DashboardComponent implements OnInit {
  ...
  loadDriverLocations(driverId: number): void {
    this.http
      .get<DriverLocation[]>(
        `${this.apiUrl}/assets/mock/driver-location/${driverId}`
      )
      .subscribe({
        next: (data) => {
  ...
  loadNearbyDrivers(lat: number, lng: number, radius: number): void {
    this.http
      .get<Driver[]>(`${this.apiUrl}/nearby-drivers`, {
  ...
  updateDriverLocation(id: string, location: any): void {
    this.http
      .put<DriverLocation>(`${this.apiUrl}/location/${id}`, location)
      .subscribe({
        next: (data) => {
          console.log('Driver Location Updated:', data);
          const index = this.driverLocations.findIndex(
            (loc) => loc.id === data.id
          );
          if (index !== -1) {
  ...
  loadStats() {
    this.driverService.getDriverStats().subscribe({
      next: (res) => {
        if (this.stats) {
          this.stats.activeDrivers = res.stats.onDutyDrivers;
        }
      },
    });
  }
