...
import { Invoice, InvoiceFilters } from '../../core/models/payment.model';
...
export class PaymentsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.loadStats();
    this.loadInvoices(1, this.itemsPerPage);
  ...
  loadInvoices(page: number = 1, limit: number = 10): void {
    this.invoiceLoadingState = true;
    
    const filters: InvoiceFilters = {};
    if (this.selectedInvoiceType) filters.type = this.selectedInvoiceType as 'driver' | 'company';
    if (this.selectedInvoiceStatus) filters.status = this.selectedInvoiceStatus as 'generated' | 'sent' | 'paid' | 'overdue';

    this.invoiceService.getInvoices(page, limit, filters).subscribe({
      next: (response) => {
        this.invoices = response.invoices;
        this.totalItems = response.totalItems;
        this.totalPages = response.totalPages;
        this.invoiceLoadingState = false;
      },
      error: (error) => {
  ...
  onInvoiceFiltersChange(): void {
    this.currentPage = 1;
    this.loadInvoices(1, this.itemsPerPage);
  }

  generateDriverInvoice(driverId?: string): void {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 7);
    const endDate = new Date();
    const targetDriverId = driverId || 'drv-001';

    this.invoiceService.generateDriverInvoice(targetDriverId, startDate, endDate).subscribe({
      next: (invoice) => {
        console.log('Driver invoice generated:', invoice);
        this.loadInvoices(1, this.itemsPerPage);
        this.loadInvoiceSummary();
      },
      error: (error) => {
        console.error('Error generating driver invoice:', error);
      }
    });
  }

  generateCompanyInvoice(): void {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 30);
    const endDate = new Date();

    this.invoiceService.generateCompanyInvoice(startDate, endDate).subscribe({
      next: (invoice) => {
        console.log('Company invoice generated:', invoice);
        this.loadInvoices(1, this.itemsPerPage);
        this.loadInvoiceSummary();
      },
      error: (error) => {
    ...
    this.invoiceService.sendInvoice(invoice.id).subscribe({
      next: (success) => {
        if (success) {
          ...
          this.loadInvoices(this.currentPage, this.itemsPerPage);
          this.loadInvoiceSummary();
        }
      },
      error: (error) => {
  ...
  markInvoiceAsPaid(invoice: Invoice): void {
  ...
  formatDateRange(period: { startDate: Date; endDate: Date }): string {
